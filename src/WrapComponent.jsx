...
export default function WrapComponent(){
    ...
    React.useEffect(()=>{
        if(JSON.parse(localStorage.getItem('Kurly_LogIn'))!==null){
            const result = JSON.parse(localStorage.getItem('Kurly_LogIn'));
            console.log(result);
            dispatch(loginINFO(result));
        }
    },[])
    ...
    //     if(sessionStorage.getItem('KURLY_ADDRESS_KEY')!==null){
    ...
    //         dispatch(address(selector.loginINFO.loginINFO.주소));
    //     }

    // },[selector.loginINFO.loginINFO]);

    React.useEffect(()=>{
        
        if(selector.loginINFO.loginINFO===null &&  sessionStorage.getItem('KURLY_ADDRESS_KEY')!==null){
            const result = JSON.parse(sessionStorage.getItem('KURLY_ADDRESS_KEY'));            
            const 주소 = {
                주소1: result.주소1,
                주소2: result.주소2
            ...
            const 주소 = {
                주소1: selector.loginINFO.loginINFO.주소.split(',')[0]===undefined?'':selector.loginINFO.loginINFO.주소.split(',')[0],
                주소2: selector.loginINFO.loginINFO.주소.split(',')[1]===undefined?'':selector.loginINFO.loginINFO.주소.split(',')[1]
            }
            dispatch(address(주소));
        }
