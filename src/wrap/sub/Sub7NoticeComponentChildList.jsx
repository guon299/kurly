...
export default function Sub7NoticeComponentChildList ({공지사항, 공지카운트, 게시글카운트}) {
    ...
    return (
        <div className="right">
                            ...
                            공지사항.map((item, idx)=>{
                                return(
                                    <li key={item.번호} onClick={(e)=>onClickList(e,item)}>
                                        <ul className='column-box'>
                                            <li className='col1'><span>{item.타입==='공지'?item.타입:게시글카운트-idx}</span></li>
            ...
            {
                selector.loginINFO.loginINFO!==null && ( // 로그인 한 경우
